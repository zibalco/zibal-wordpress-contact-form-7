...
function ZIBAL_CF7_relative_time($ptime)
{
...
function result_payment_func($atts)
{
    ...
    if ($Status == '2') {
        ...
        $cf_Form = $wpdb->get_row("SELECT * FROM $table_name WHERE transid=" . $Return_Track_Id);
        if (null !== $cf_Form) {
            $Amount = $cf_Form->cost;
        }
    ...
    if ($Return_MessageEmail == 'success') {
        $wpdb->update($wpdb->prefix . 'zibal_contact_form_7', array('status' => 'success', 'transid' => $result->refNumber), array('transid' => $Return_Track_Id), array('%s', '%s'), array('%d'));

        //Dispaly
        $body = '<b style="color:'.$sucess_color.';">'.stripslashes(str_replace('[transaction_id]', $result->refNumber, $Theme_Message)).'<b/>';
        return CreateMessage_cf7("", "", $body);
    } else if ($Return_MessageEmail == 'error') {
        $wpdb->update($wpdb->prefix . 'zibal_contact_form_7', array('status' => 'error'), array('transid' => $Return_Track_Id), array('%s'), array('%d'));
        //Dispaly
...
function ZIBAL_Contant_Form_7_Gateway_install()
{
...
//register_activation_hook($dir . 'gateway_func.php', 'ZIBAL_Contant_Form_7_Gateway_install');
    ...
    function cf7pp_admin_list_trans()
    {
        ...
        if (count($transactions) == 0) {
            ...
            foreach ($transactions as $transaction) {
                ...
                echo '<br />(';
                echo ZIBAL_CF7_relative_time($transaction["created_at"]);
                echo ' قبل)</td>';
